...
\begin{itemize}
   ...
   %
   \item \textit{variance/MSE estimation [testing phase; floating-point arithmetic problems]}
   %
...
\noindent (Robust estimating methods for model B will be released in the near future)
...
To learn more about the fit, we may call the \texttt{summary} method. The properties of the model summary will be discussed in the context of Huber-type $M$-estimation (see below). 

%Another important tool is the \texttt{residuals} function. It extracts the residuals (\texttt{res}), the standardized residuals (\texttt{stdres=}$\matv^{-1/2}$ \texttt{res}), and the robustness weights, \texttt{wgt}, of the Huber $\psi$-function associated with \texttt{stdres}.
...
To learn more about the fit, namely about the number of iterations and the robustness properties, we shall call the \texttt{summary} method.

<<echo=TRUE>>=
summary(huberfit)
@

\noindent The output is by default separated into 4 blocks when the algorithm converged. In the case of non-convergence, the report is slightly adapted. 

\begin{itemize}
   \item Block 1 reports general information on the method and the robustness tuning constant(s).
   \item Block 2 reports the degree of down-weighting outlying residuals at the final iteration. 

%Denote by $\vbeta^{(s)}$ the value of the fixed-effect coefficient vector at the $s$th (say, final) iteration. The definitions for $v_e^{(s)}$ and $v_u^{(s)}$, the model-error variance and (area-level) random-effect variance estimates, respectively, are analogous. Now, denote by $r_{i,j}^{(s)}$ the (standardized) residual computed with the parameter estimates at the $s$th iteration (i.e., $ \vbeta^{(s)}, v_e^{(s)}, v_u^{(s)}$), $j=1,\ldots,n_i$, $i=1,\ldots,g$. The degree of down-weighting, $d$, is defined as

%\begin{equation}
%   d =  \biggl[\sum_{i=1}^g n_i \biggr]^{-1}\sum_{i=1}^g\sum_{j=1}^{n_i} \frac{\psi_k\bigl[r_{i,j}^{(s)}\bigr]}{r_{i,j}^{(s)}}, \nonumber
%\end{equation}

%where $\psi_k(\cdot)$ is the Huber $\psi$-function, indexed by the robustness tuning constant $k$.
The degree of down-weighting is reported for all estimating equations (EE) separately. In general $d=1$, if no down-weighting took place. Conversely, the smaller $d$, the more outlying residuals have been down-weighted (and/or the same outliers are heavier down-weighted).
   ...
   \item Block 3 reports the default or user-specified max number of iterations, \texttt{niter}, and the numeric tolerance, \texttt{acc}, used in the termination rule of the algorithms. 
   \item Block 4 reports the number of iterations that each of the estimating equation--specific (EE-specific) loops (aka inner loops) and the overall loop (aka outer loop) used to conform to the termination rule. Each row in the table represents a single run of the overall loop. In the example, the algorithm needed 7 overall loops. The entries of a row refer to the EE-specific number of iterations in a particular overall loop. 
...
However, \citet{schoch2011} showed that robust predictions of the random effects can be obtained far more easily. Namely, since the robustly estimated parameters of the core Gaussian model determine the model completely, prediction is straightforward.
...
\noindent where \texttt{fit} is a fitted model (an object produced by \texttt{fitsaemodel}),  \texttt{k} is the robustness-tuning constant (of the Huber $\psi$-function) for robust prediction. The robustness-tuning constant \texttt{k} does not necessarily be the same as the one used in \texttt{fitsaemodel}. Further, choosing \texttt{k} sufficiently large (e.g., \texttt{k=20000} should work in (almost) all situations), \texttt{robpredict} produces the usual EBLUP predictions. If \texttt{areadata=NULL} (the default setting), the prediction is based on the same data that have been used for the model fitting exercise (i.e., within-sample prediction). However, in the SAE context, we are usually interested in (robustly) predicting the small-area means. Therefore, we deliver the area-specific population means through  \texttt{areadata}.

In the \texttt{landsat} example, the county-specific population means of pixels of the segments under corn and soybeans are recorded in the variables \texttt{MeanPixelsCorn} and \texttt{MeanPixelsSoybeans}, respectively. Note that each sample segment in a particular county has assigned the county-specific mean (in the \texttt{landsat} data). Therefore, each population mean of the variables \texttt{MeanPixelsCorn} and \texttt{MeanPixelsSoybeans} occurs $n_i$ times. The unique county-specific are obtained using 
...
Next, we consider predicting the random and fixed effects and the county means. Note that we choose \texttt{k=10000}. This leads to results that are equivalent to those of the standard EBLUP, and therefore not robust.

<<>>=
pr <- robpredict(mlfit, k=10000, areadata=d)
@
